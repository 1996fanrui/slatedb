...
impl DbInner {
    ...
    pub(crate) async fn maybe_apply_backpressure(&self) -> Result<(), SlateDBError> {
        loop {
            ...
            let total_mem_size_bytes = {
                let guard = self.state.read();
                // Size of the active memtable plus every immutable memtable. Each read is an
                // atomic load, so the active memtable is read under the read lock.
                guard
                    .state()
                    .imm_memtable
                    .iter()
                    .map(|imm| imm.table().metadata())
                    .chain(std::iter::once(guard.memtable().table().metadata()))
                    .map(|metadata| {
            ...
            trace!(
                "checking backpressure [total_mem_size_bytes={}, max_unflushed_bytes={}]",
            ...
            if total_mem_size_bytes >= self.settings.max_unflushed_bytes {
                ...
                warn!(
                    "unflushed memtable size exceeds max_unflushed_bytes. applying backpressure. [total_mem_size_bytes={}, max_unflushed_bytes={}]",
                    format_bytes_si(total_mem_size_bytes as u64),
                    format_bytes_si(self.settings.max_unflushed_bytes as u64),
                );
...
mod tests {
    ...
    async fn test_should_record_total_mem_size_bytes_with_wal_disabled() {
        ...
        // then: total_mem_size_bytes reflects the active memtable
